...
struct block_meta *find_free_block(size_t size, int param, struct block_meta *stop)
{
	...
	if (closest_block) {
		...
		if (PADDING(closest_block->size) - padded_size >= (size_t)(padded_meta + 8)) {
...
struct block_meta *request_space(struct block_meta *last, size_t size)
{
	...
	if (request_size < allocation_threshold) {
		...
		block->prev = last;
		block->size = (size % 8 == 0) ? size : (size + (8 - size % 8));
		block->next = NULL;
	...
	void *ptr = mmap(NULL, request_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONN, -1, 0);
	...
	((struct block_meta *)ptr)->size = (size % 8 == 0) ? size : (size + (8 - size % 8));
...
void *os_malloc(size_t size)
{
	...
	//daca marimea este mai mica decat mmap threshold vom folosi brk/sbrk pentru alocarea de memorie
		...
		if (!block_meta_head && heap_prealocatted == 1) {
			...
			if (PADDING(size) >= (size_t)allocation_threshold)
				block = NULL;

			if (PADDING(size) >= (size_t)allocation_threshold && allocation_threshold == CALLOC_THRESHOLD
			&& coming_from_calloc == 1) {
				//cer memorie noua cu mmap
				int calloc_request_size = META_PADDING + PADDING(size);
				void *calloc_ptr = mmap(NULL, calloc_request_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONN, -1, 0);
	...
	if (size > current_block->size) {
		...
		if (current_block->next != NULL && current_block->next->status == STATUS_FREE &&
		 PADDING(current_block->size) + META_PADDING + PADDING(current_block->next->size) >= PADDING(size)) {
			...
			struct block_meta *optimal_find = find_free_block(size, 1, current_block);
		...
		if (last_non_mmap->status == STATUS_FREE && find_free_block(size, 0, NULL) == NULL) {
